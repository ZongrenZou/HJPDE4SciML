"""Code for example 4: the third equation"""
...
c3 = np.zeros(shape=[10, 1], dtype=np.float64)
tau = 0.5
sigma = 0.5
...
for i in range(20000):
    # step 1
    _b = tau * np.matmul(phi.T, u_t[:, 2:3]) + c3 - tau * y
    ...
    u_t_pred = np.matmul(phi, c3)
    fs += [0.5 * np.sum((u_t_pred - u_t[:, 2:3]) ** 2) + eps * np.sum(np.abs(c3))]
c3_ref = c3
print("Identified coefficients:")
print(np.round(c3_ref.flatten(), 8))
...
for i in range(u_t.shape[0]):
    P, q = methods.update_RK4(
        P2s[-1], q2s[-1], h, phi[i : i + 1, :], u_t[i : i + 1, 2:3], lamb=1, tau=tau
    )
    P2s += [P]
    q2s += [q]
P, q = P, q
...
###################### Results ######################
print("L1: ", np.linalg.norm(c3 - c3_ref, 1))
print("L1 relative: ", np.linalg.norm(c3 - c3_ref, 1) / np.linalg.norm(c3_ref, 1))
print("Identified coefficients:")
print(np.round(c3.flatten(), 8))
